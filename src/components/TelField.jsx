 ...
 */
export default function TelField({ value = ['', '', ''], onChange }) {
  const setPart = (idx) => (e) => {
    const next = [...value]
  ...
  return (
    <div className="grid min-w-0 w-full grid-cols-[minmax(0,1fr)_auto_minmax(0,1fr)_auto_minmax(0,1fr)] items-center gap-1.5">
      <input className={inputClass} value={value[0]} onChange={setPart(0)} maxLength={3} />
      <span className="text-gray-300">-</span>
      <input className={inputClass} value={value[1]} onChange={setPart(1)} maxLength={4} />
      <span className="text-gray-300">-</span>
      <input className={inputClass} value={value[2]} onChange={setPart(2)} maxLength={4} />
    </div>
  )
}
